...
-(CAShapeLayer *)allClickedLayer
{
    if (!_allClickedLayer) {
        ...
        _allClickedLayer.fillColor = [UIColor colorWithWhite:0.5 alpha:0.3].CGColor;
...
- (void)setSelectedRange:(NSRange)range
{
    ...
    if (self.selectedRange.length && !NSEqualRanges(self.selectedRange, range)) {
        ...
        [self.textStorage addAttribute:NSForegroundColorAttributeName
                                 value:[self getLinkColorWithRange:self.selectedRange]
                                 range:self.selectedRange];
    }
...
- (void)updateTextStoreWithAttributedString:(NSAttributedString *)attributedString
{
    ...
    if (self.textStorage) {
        ...
        //[self.layoutManager setTextStorage:self.textStorage];//和上句代码作用一样，重复
    }
}
/*
 * 链接文本属性
...
- (NSAttributedString *)addLinkAttributesToAttributedString:(NSAttributedString *)string linkRanges:(NSArray *)linkRanges
{
    ...
    for (NSDictionary *dictionary in linkRanges)
    {
        ...
        if ([[dictionary allKeys] count] == 4) {
            NSDictionary *attdic = [dictionary objectForKey:@"attDic"];
            [attributedString addAttributes:attdic range:range];
        }
        else
        {
...
- (NSArray *)getRangesForLinks:(NSAttributedString *)text
{
    ...
    if (self.linkDetectionTypes & KZLinkDetectionTypeCustom)
    {
        ...
        [rangesForLinks addObjectsFromArray:self.customLinkArray];
    }
...
-(void)setLinkNSRange:(NSRange)range WithTag:(NSInteger)tag
{
    if (!_customLinkArray) {
        _customLinkArray = [NSMutableArray new];
    }
    
    [_customLinkArray addObject:@{
                                      @"linkType" : @(KZLinkDetectionTypeCustom),
                                      @"range"    : [NSValue valueWithRange:range],
                                      @"link"     : [NSString stringWithFormat:@"%ld",tag]
                                      }];
    
}

-(void)setLinkNSRange:(NSRange)range WithTag:(NSInteger)tag AndAttributed:(NSDictionary *)attDic
{
    if (!_customLinkArray) {
        _customLinkArray = [NSMutableArray new];
    }
...
-(UIColor *)getLinkColorWithRange:(NSRange)range
{
    for (NSDictionary *tmpDic in self.customLinkArray) {
        
        if ([[tmpDic allKeys] count] != 4) {
            continue;
        }
        
        NSValue *va1 = [tmpDic objectForKey:@"range"];
        NSValue *va2 = [NSValue valueWithRange:range];
        
        if (![va1 isEqualToValue:va2]) {
            continue;
        }
        
        NSDictionary *attDic = [tmpDic objectForKey:@"attDic"];
        return [attDic objectForKey:NSForegroundColorAttributeName];
...
-(void)setCustomLinkArray:(NSMutableArray *)customLinkArray
{
    if (!_customLinkArray) {
        _customLinkArray = [NSMutableArray new];
    }
    
    for (NSDictionary *dic in customLinkArray)
    {
        
        [_customLinkArray addObject:@{
                                      @"linkType" : @(KZLinkDetectionTypeCustom),
                                      @"range"    : [dic objectForKey:@"rang"],
                                      @"link"     : [dic objectForKey:@"tag"]
                                      }];
...
-(UIBezierPath *)getTextPath
{
    NSRange lineRange;
    CGRect lineRect = [self.layoutManager lineFragmentUsedRectForGlyphAtIndex:self.attributedText.string.length-1 effectiveRange:&lineRange];
    CGSize size = self.bounds.size;
    CGSize linesize = lineRect.size;
    UIBezierPath *path = [UIBezierPath bezierPath];
    [path moveToPoint:CGPointMake(0, 0)];
    [path addLineToPoint:CGPointMake(size.width, 0)];
    [path addLineToPoint:CGPointMake(size.width, size.height-linesize.height)];
    [path addLineToPoint:CGPointMake(linesize.width, size.height-linesize.height)];
    [path addLineToPoint:CGPointMake(linesize.width, size.height)];
...
-(NSRange)getSelfStrRange
{
    return NSMakeRange(0, self.attributedText.string.length);
}
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (touchedLink){
        ...
        if (self.canAllClicked) {
            if (!backPath) {
                backPath = [self getTextPath];
                self.allClickedLayer.path = backPath.CGPath;
                [self.layer addSublayer:self.allClickedLayer];
            }
            self.allClickedLayer.hidden = NO;
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (touchedLink){
        ...
        if (self.canAllClicked) {
            self.linkTapHandler(KZLinkTypeAllClicked, self.attributedText.string, NSMakeRange(0, self.attributedText.string.length));
        }
        else
        {
            [super touchesBegan:touches withEvent:event];
...
-(void)removeBackView
{
    self.allClickedLayer.hidden = YES;
}
